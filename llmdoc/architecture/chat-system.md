...
- `app/api/chat/route.ts` (`POST`, `analyzeBazi`, `generateMascot`, `retextureMascot`, `presentOptions`): 服务端 API 路由，使用 `streamText` 调用 DeepSeek 模型，定义四个工具并返回 `UIMessageStreamResponse`。系统提示词为全中文，AI 人设为"铁口直断"的年轻命理师，采用原则驱动式提示词设计。
...
3. **API 处理：** `app/api/chat/route.ts:106-163` -- POST handler 解构 `{ messages, pendingTaskId }`，调用 `streamText` 配置 DeepSeek 模型和四个工具（analyzeBazi、generateMascot、retextureMascot、presentOptions）。
4. **流式响应：** `app/api/chat/route.ts:162` -- `result.toUIMessageStreamResponse()` 将流转换为 UI 消息格式。
...
2. **Parts 路由：** `components/chat/chat-message.tsx:56-130` -- 遍历 `message.parts`，按 `type` 分发：
   ...
   - `tool-presentOptions`（output-available）-> `OptionsButtons`（选项按钮组，点击发送消息）
...
- **presentOptions：** `app/api/chat/route.ts:12-21` -- 接收选项列表（label + 可选 description），直接返回。前端由 `OptionsButtons` 渲染按钮组，点击时通过 `onSendMessage` 向聊天发送消息。
